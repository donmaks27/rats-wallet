...
function startAction(user, userData, args, callback) {
    ...
    db.category_delete(categoryID, (error) => {
        if (error) {
            log.error(userID, `failed to delete category ${categoryID} (${error})`);
        }
        ...
        bot.editMessage({ 
            ...
            text: error ? `_Something went wrong, failed to delete category_` : `_Category deleted_`,
...
function stopAction(user, userData, args, callback) {
    const userID = user.id;
    log.info(userID, `switching to categories menu...`);
    walletMenu.changeMenuMessage(walletCommon.getUserMenuMessageID(userID), 'categories', { categoryID: args.parentCategoryID }, user, userData, (message, error) => {
        if (error) {
            log.error(userID, `failed to switch to categories menu (${error})`);
        } else {
            log.info(userID, `switched to categories menu`);
        }
        callback(true);
    });
}
